...
/*
 * Primary Processor-based VM Execution Control capabilities
...
/*
 * Secondary Processor-based VM Execution Control capabilities
 * See SDM volume 3, section 24.6.2
 */
struct capability_info secondary_proc_based_control[27] = 
{
...
detect_vmx_features(void)
{
	...
	pr_info("Secondary Processor based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(secondary_proc_based_control, 27, lo, hi);
}
